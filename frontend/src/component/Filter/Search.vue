<template>
  ...
  >
    ...
    <div class="flex items-center w-full min-w-0">
      ...
      <div
        class="filter-search__controls relative flex flex-col w-full min-w-0"
        :class="{ 'filter-search__controls--stacked': breakHg, 'sm:flex-row': !breakHg && !veryBig }"
      >
        <b-select
          v-if="showFields === 'select'"
          class="filter-search__select w-full"
          :maxWidth="110"
          :color="color"
          :options="fields"
        ...
        <div class="filter-search__input-wrap flex w-full min-w-0">
          <!-- text -->
          <input
            class="filter-search__input flex-1 min-w-0 outline-none border-gray border-base border-r-0 placeholder-darkGray bg-white"
            :class="`${ focusStyle } ${ veryBig ? 'py-mmd' : (big ? 'py-sm' : 'py-xxs') }
              ${ autoFocus ? ' auto-focus' : ''}
              ${ veryBig ? 'text-mmd px-md rounded-l-2xl' : 'text-md px-sm rounded-l-2xl' }`"
            type="text"
          ...
          <!-- send -->
          <button
            class="filter-search__submit py-xs transition-all duration-300 focus:outline-none"
            :class="sendButtonActiveStyle + ' ' + (veryBig ? 'px-lg text-mmd rounded-r-2xl' : 'px-base text-sm rounded-r-2xl')"
...
const placeholder: any = $computed(() => props.useCurrentSearch ? 'Search resources...' : 'Search in resources...');
...
const sendButtonActiveStyle: string = $computed(() => {
  if (canUseSendButton) {
    return `bg-${ props.color } border-${ props.color } ${ props.hoverStyle } border-base shadow-none`;
  }
